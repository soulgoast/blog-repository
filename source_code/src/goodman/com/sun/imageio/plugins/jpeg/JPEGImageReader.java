...
import goodman.javax.imageio.ImageReadParam;
...
import goodman.javax.imageio.plugins.jpeg.JPEGImageReadParam;
...
import goodman.java.awt.color.CMMException;
...
public class JPEGImageReader extends ImageReader {
    ...
    private com.sun.imageio.plugins.jpeg.JPEGMetadata streamMetadata = null;

    /** Image metadata, valid for the imageMetadataIndex only. */
    private com.sun.imageio.plugins.jpeg.JPEGMetadata imageMetadata = null;
    ...
    private void checkTablesOnly() throws IOException {
        ...
        if (tablesOnly) {
            ...
            if (ignoreMetadata == false) {
                iis.seek(savePos);
                haveSeeked = true;
                streamMetadata = new com.sun.imageio.plugins.jpeg.JPEGMetadata(true, false,
                                                  iis, this);
                long pos = iis.getStreamPosition();
                if (debug) {
                    System.out.println
      ...
      throws IOException {
        ...
        if (allowSearch == true) {
            ...
            while (!done) {
                ...
                case com.sun.imageio.plugins.jpeg.JPEG.SOI:
                    numImages++;
    ...
    private void skipImage() throws IOException {
        ...
        for (int byteval = iis.read();
            ...
            if (foundFF == true) {
                if (byteval == com.sun.imageio.plugins.jpeg.JPEG.EOI) {
                    return;
                }
            }
    ...
    private boolean hasNextImage() throws IOException {
        ...
        for (int byteval = iis.read();
            ...
            if (foundFF == true) {
                if (byteval == com.sun.imageio.plugins.jpeg.JPEG.SOI) {
                    iis.reset();
                    if (debug) {
    ...
    private ImageTypeProducer getImageType(int code) {
        ...
        if ((code > 0) && (code < com.sun.imageio.plugins.jpeg.JPEG.NUM_JCS_CODES)) {
    ...
    private Iterator getImageTypesOnThread(int imageIndex)
        ...
        case com.sun.imageio.plugins.jpeg.JPEG.JCS_GRAYSCALE:
            list.add(raw);
            list.add(getImageType(com.sun.imageio.plugins.jpeg.JPEG.JCS_RGB));
            break;
        case com.sun.imageio.plugins.jpeg.JPEG.JCS_RGB:
            list.add(raw);
            list.add(getImageType(com.sun.imageio.plugins.jpeg.JPEG.JCS_GRAYSCALE));
            list.add(getImageType(com.sun.imageio.plugins.jpeg.JPEG.JCS_YCC));
            break;
        case com.sun.imageio.plugins.jpeg.JPEG.JCS_RGBA:
            list.add(raw);
            break;
        case com.sun.imageio.plugins.jpeg.JPEG.JCS_YCC:
            if (raw != null) {  // Might be null if PYCC.pf not installed
                list.add(raw);
                list.add(getImageType(com.sun.imageio.plugins.jpeg.JPEG.JCS_RGB));
            }
            break;
        case com.sun.imageio.plugins.jpeg.JPEG.JCS_YCCA:
            if (raw != null) {  // Might be null if PYCC.pf not installed
                list.add(raw);
            }
            break;
        case com.sun.imageio.plugins.jpeg.JPEG.JCS_YCbCr:
                ...
                list.add(new ImageTypeProducer() {
                    protected ImageTypeSpecifier produce() {
                        return ImageTypeSpecifier.createInterleaved
                         (iccCS,
                          com.sun.imageio.plugins.jpeg.JPEG.bOffsRGB,  // Assume it's for RGB
            ...
            list.add(getImageType(com.sun.imageio.plugins.jpeg.JPEG.JCS_YCC));
            break;
        case com.sun.imageio.plugins.jpeg.JPEG.JCS_YCbCrA:  // Default is to convert to RGBA
            // As there is no YCbCr ColorSpace, we can't support
            // the raw type.
            list.add(getImageType(com.sun.imageio.plugins.jpeg.JPEG.JCS_RGBA));
            break;
        }
    ...
    private void checkColorConversion(BufferedImage image,
        ...
        case com.sun.imageio.plugins.jpeg.JPEG.JCS_GRAYSCALE:  // Its gray in the file
            if  (csType == ColorSpace.TYPE_RGB) { // We want RGB
                // IJG can do this for us more efficiently
                setOutColorSpace(structPointer, com.sun.imageio.plugins.jpeg.JPEG.JCS_RGB);
                // Update java state according to changes
                // in the native part of decoder.
                outColorSpaceCode = com.sun.imageio.plugins.jpeg.JPEG.JCS_RGB;
                numComponents = 3;
            } else if (csType != ColorSpace.TYPE_GRAY) {
                throw new IIOException("Incompatible color conversion");
            }
            break;
        case com.sun.imageio.plugins.jpeg.JPEG.JCS_RGB:  // IJG wants to go to RGB
            if (csType ==  ColorSpace.TYPE_GRAY) {  // We want gray
                if (colorSpaceCode == com.sun.imageio.plugins.jpeg.JPEG.JCS_YCbCr) {
                    // If the jpeg space is YCbCr, IJG can do it
                    setOutColorSpace(structPointer, com.sun.imageio.plugins.jpeg.JPEG.JCS_GRAYSCALE);
                    // Update java state according to changes
                    // in the native part of decoder.
                    outColorSpaceCode = com.sun.imageio.plugins.jpeg.JPEG.JCS_GRAYSCALE;
                    numComponents = 1;
                }
            } else if ((iccCS != null) &&
                ...
                convert = new ColorConvertOp(com.sun.imageio.plugins.jpeg.JPEG.JCS.sRGB, cs, null);
        ...
        case com.sun.imageio.plugins.jpeg.JPEG.JCS_YCC:
            {
                ColorSpace YCC = com.sun.imageio.plugins.jpeg.JPEG.JCS.getYCC();
    ...
    private Raster readInternal(int imageIndex,
        ...
        int [] srcBands = com.sun.imageio.plugins.jpeg.JPEG.bandOffsets[numComponents-1];
        ...
        int [] bandOffs = com.sun.imageio.plugins.jpeg.JPEG.bandOffsets[numRasterBands-1];
...
class ImageTypeProducer {
    ...
    private static final ImageTypeProducer [] defaultTypes =
            new ImageTypeProducer [com.sun.imageio.plugins.jpeg.JPEG.NUM_JCS_CODES];

    public synchronized static ImageTypeProducer getTypeProducer(int csCode) {
        if (csCode < 0 || csCode >= com.sun.imageio.plugins.jpeg.JPEG.NUM_JCS_CODES) {
            return null;
        }
    ...
    protected ImageTypeSpecifier produce() {
        switch (csCode) {
            ...
            case com.sun.imageio.plugins.jpeg.JPEG.JCS_RGB:
                return ImageTypeSpecifier.createInterleaved(com.sun.imageio.plugins.jpeg.JPEG.JCS.sRGB,
                        com.sun.imageio.plugins.jpeg.JPEG.bOffsRGB,
                        DataBuffer.TYPE_BYTE,
                        false,
                        false);
            case com.sun.imageio.plugins.jpeg.JPEG.JCS_RGBA:
                return ImageTypeSpecifier.createPacked(com.sun.imageio.plugins.jpeg.JPEG.JCS.sRGB,
                        0xff000000,
                        0x00ff0000,
                        0x0000ff00,
                        0x000000ff,
                        ...
                        false);
            case com.sun.imageio.plugins.jpeg.JPEG.JCS_YCC:
                if (com.sun.imageio.plugins.jpeg.JPEG.JCS.getYCC() != null) {
                    return ImageTypeSpecifier.createInterleaved(
                            com.sun.imageio.plugins.jpeg.JPEG.JCS.getYCC(),
                        com.sun.imageio.plugins.jpeg.JPEG.bandOffsets[2],
                        DataBuffer.TYPE_BYTE,
                        false,
                        false);
                } else {
