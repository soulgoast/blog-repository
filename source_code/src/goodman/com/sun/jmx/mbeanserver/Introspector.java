...
import goodman.java.lang.reflect.AnnotatedElement;
...
import goodman.javax.management.DescriptorKey;
...
public class Introspector {
    ...
    public static <T> DynamicMBean makeDynamicMBean(T mbean)
        ...
        try {
            c = com.sun.jmx.mbeanserver.Util.cast(getStandardMBeanInterface(mbeanClass));
        } catch (NotCompliantMBeanException e) {
        ...
        try {
            c = com.sun.jmx.mbeanserver.Util.cast(getMXBeanInterface(mbeanClass));
        } catch (NotCompliantMBeanException e) {
    ...
    private static <T> Class<? super T> findMBeanInterface(
        ...
        while (current != null) {
            ...
            for (int i=0;i<len;i++)  {
                Class<? super T> inter = com.sun.jmx.mbeanserver.Util.cast(interfaces[i]);
