...
import goodman.java.util.concurrent.locks.ReentrantReadWriteLock;
...
public class Repository {
            ...
            if (isPropertyValuePattern || isPropertyListPattern) {
                for (int i = keys.length - 1; i >= 0 ; i--) {
                    ...
                    if (isPropertyValuePattern &&
                        ...
                        // v is the string
                        if (com.sun.jmx.mbeanserver.Util.wildmatch(v,values[i]))
                            continue;
                        else
                            return false;
                    }
    ...
    public void addMBean(final DynamicMBean object, ObjectName name,
        ...
        if (dom.length() == 0)
            name = com.sun.jmx.mbeanserver.Util.newObjectName(domain + name.toString());
