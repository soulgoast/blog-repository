 ...
 * <p>
 * ObjectOutputStream可以将原始数据类型和对象结构写入到一个输出流中。也可以使用ObjectInputStream将对象二进制流还原成java对象。
 * 可以使用一个文件或者流来持久化存储对象。如果流是一个网络流，该对象可以在其他的主机或者进程中被还原。
 * </p>
 *
 ...
 * 只有对象继承了Serializable接口才能被写入流中。
 * 每个可序列化对象的类都经过编码，包括类的名称和签名，对象的字段和数组的值以及从初始对象中引用的所有其他对象的关闭。
 *
 ...
 * <p>
 *     writeObject方法用于将对象写入流。任何对象，包括字符串和数组，都是使用writeObject编写的。
 *     可以将多个*对象或基元写入流中。必须以相同的类型并以与写入对象相同的顺序从相应的ObjectInputstream中读取对象。
 * </p>
 *
 *
 ...
 * writeutf method.
 * 还可以使用dataoutput中的适当方法将原始数据类型写入流中。字符串也可以使用writeutf方法编写。
 *
    ...
    extends java.io.OutputStream implements java.io.ObjectOutput, ObjectStreamConstants
{
    ...
    /**
     * if true, invoke writeObjectOverride() instead of writeObject() <br>
     * 如果为true，调用writeObjectOverriade()代替writeObject()方法 <br>
     */
