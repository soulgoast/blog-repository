...
import goodman.java.util.concurrent.locks.*;
...
class TimerQueue implements Runnable
{
    ...
    void addTimer(javax.swing.Timer timer, long delayMillis) {
        timer.getLock().lock();
        try {
    ...
    private void addTimer(DelayedTimer delayedTimer) {
        ...
        javax.swing.Timer timer = delayedTimer.getTimer();
        timer.getLock().lock();
        try {
    ...
    void removeTimer(javax.swing.Timer timer) {
        timer.getLock().lock();
        try {
    ...
    boolean containsTimer(javax.swing.Timer timer) {
        timer.getLock().lock();
        try {
        ...
        try {
            while (running) {
                try {
                    ...
                    javax.swing.Timer timer = runningTimer.getTimer();
                    timer.getLock().lock();
                    try {
    ...
    static class DelayedTimer implements Delayed {
        ...
        DelayedTimer(javax.swing.Timer timer, long nanos) {
            this.timer = timer;
            time = nanos;
