...
import goodman.javax.swing.plaf.nimbus.SynthPainterImpl;
...
public final class NimbusStyle extends SynthStyle {
    ...
    private void validate() {
        ...
        List<javax.swing.plaf.nimbus.State> states = new ArrayList<javax.swing.plaf.nimbus.State>();
        ...
        if (statesString != null) {
            ...
            for (int i=0; i<s.length; i++) {
                s[i] = s[i].trim();
                if (!javax.swing.plaf.nimbus.State.isStandardStateName(s[i])) {
                    //this is a non-standard state name, so look for the
                    //custom state associated with it
                    String stateName = prefix + "." + s[i];
                    javax.swing.plaf.nimbus.State customState = (javax.swing.plaf.nimbus.State)defaults.get(stateName);
                    if (customState != null) {
                        states.add(customState);
                    }
                } else {
                    states.add(javax.swing.plaf.nimbus.State.getStandardState(s[i]));
                }
            }
            ...
            if (states.size() > 0) {
                values.stateTypes = states.toArray(new javax.swing.plaf.nimbus.State[states.size()]);
            }

            //assign codes for each of the state types
            int code = 1;
            for (javax.swing.plaf.nimbus.State state : states) {
            ...
            states.add(javax.swing.plaf.nimbus.State.Enabled);
            states.add(javax.swing.plaf.nimbus.State.MouseOver);
            states.add(javax.swing.plaf.nimbus.State.Pressed);
            states.add(javax.swing.plaf.nimbus.State.Disabled);
            states.add(javax.swing.plaf.nimbus.State.Focused);
            states.add(javax.swing.plaf.nimbus.State.Selected);
            states.add(javax.swing.plaf.nimbus.State.Default);
        ...
        if (property != null) {
            ...
            if (v.stateTypes == null){
                ...
                for (String stateStr : states) {
                    javax.swing.plaf.nimbus.State.StandardState s = javax.swing.plaf.nimbus.State.getStandardState(stateStr);
                    if (s != null) xstate |= s.getState();
                }
            } else {
                // custom states
                for (javax.swing.plaf.nimbus.State s : v.stateTypes) {
