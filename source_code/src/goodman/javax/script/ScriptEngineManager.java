...
package goodman.javax.script;
import goodman.javax.script.Bindings;
import goodman.javax.script.ScriptContext;
import goodman.javax.script.ScriptEngine;
import goodman.javax.script.ScriptEngineFactory;
import goodman.javax.script.SimpleBindings;
...
public class ScriptEngineManager  {
    ...
    private void init(final ClassLoader loader) {
        ...
        engineSpis = new HashSet<javax.script.ScriptEngineFactory>();
        nameAssociations = new HashMap<String, javax.script.ScriptEngineFactory>();
        extensionAssociations = new HashMap<String, javax.script.ScriptEngineFactory>();
        mimeTypeAssociations = new HashMap<String, javax.script.ScriptEngineFactory>();
        initEngines(loader);
    }

    private ServiceLoader<javax.script.ScriptEngineFactory> getServiceLoader(final ClassLoader loader) {
        if (loader != null) {
            return ServiceLoader.load(javax.script.ScriptEngineFactory.class, loader);
        } else {
            return ServiceLoader.loadInstalled(javax.script.ScriptEngineFactory.class);
        }
    }

    private void initEngines(final ClassLoader loader) {
        Iterator<javax.script.ScriptEngineFactory> itr = null;
        try {
            ServiceLoader<javax.script.ScriptEngineFactory> sl = AccessController.doPrivileged(
                new PrivilegedAction<ServiceLoader<javax.script.ScriptEngineFactory>>() {
                    @Override
                    public ServiceLoader<javax.script.ScriptEngineFactory> run() {
        ...
        try {
            while (itr.hasNext()) {
                try {
                    javax.script.ScriptEngineFactory fact = itr.next();
                    engineSpis.add(fact);
                } catch (ServiceConfigurationError err) {
    ...
    public javax.script.ScriptEngine getEngineByName(String shortName) {
        ...
        if (null != (obj = nameAssociations.get(shortName))) {
            javax.script.ScriptEngineFactory spi = (javax.script.ScriptEngineFactory)obj;
            try {
                javax.script.ScriptEngine engine = spi.getScriptEngine();
                engine.setBindings(getBindings(), javax.script.ScriptContext.GLOBAL_SCOPE);
                return engine;
            } catch (Exception exp) {
        ...
        for (javax.script.ScriptEngineFactory spi : engineSpis) {
    ...
    public List<javax.script.ScriptEngineFactory> getEngineFactories() {
        List<javax.script.ScriptEngineFactory> res = new ArrayList<javax.script.ScriptEngineFactory>(engineSpis.size());
        for (javax.script.ScriptEngineFactory spi : engineSpis) {
            res.add(spi);
        }
     ...
     */
    public void registerEngineName(String name, javax.script.ScriptEngineFactory factory) {
     ...
     * @throws NullPointerException if any of the parameters is null.
     */
    public void registerEngineMimeType(String type, javax.script.ScriptEngineFactory factory) {
     ...
     * @throws NullPointerException if any of the parameters is null.
     */
    public void registerEngineExtension(String extension, javax.script.ScriptEngineFactory factory) {
        if (extension == null || factory == null) throw new NullPointerException();
        extensionAssociations.put(extension, factory);
    }

    /** Set of script engine factories discovered. */
    private HashSet<javax.script.ScriptEngineFactory> engineSpis;

    /** Map of engine name to script engine factory. */
    private HashMap<String, javax.script.ScriptEngineFactory> nameAssociations;

    /** Map of script file extension to script engine factory. */
    private HashMap<String, javax.script.ScriptEngineFactory> extensionAssociations;
