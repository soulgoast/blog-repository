...
import goodman.javax.management.JMException;
import goodman.javax.management.JMRuntimeException;
import goodman.javax.management.MBeanPermission;
import goodman.javax.management.MBeanServer;
import goodman.javax.management.MBeanServerBuilder;
import goodman.javax.management.MBeanServerDelegate;
import goodman.javax.management.MBeanServerPermission;
import goodman.javax.management.loading.ClassLoaderRepository;
...
public class MBeanServerFactory {
     ...
     *
     **/
    private static javax.management.MBeanServerBuilder builder = null;
     ...
     *
     * @exception javax.management.JMRuntimeException if the property
     ...
     * null from its {@link javax.management.MBeanServerBuilder#newMBeanServerDelegate
     * newMBeanServerDelegate} or {@link
     * javax.management.MBeanServerBuilder#newMBeanServer newMBeanServer} methods.
     *
    ...
    /**
     * <p>Return a new object implementing the {@link javax.management.MBeanServer}
     ...
     * javax.management.MBeanServerBuilder}.
     */
    public static javax.management.MBeanServer createMBeanServer(String domain)  {
        checkPermission("createMBeanServer");

        final javax.management.MBeanServer mBeanServer = newMBeanServer(domain);
     ...
     * javax.management.MBeanServerBuilder}.
     */
    public static javax.management.MBeanServer newMBeanServer(String domain)  {
        checkPermission("newMBeanServer");

        // Get the builder. Creates a new one if necessary.
        //
        final javax.management.MBeanServerBuilder mbsBuilder = getNewMBeanServerBuilder();
        ...
        synchronized(mbsBuilder) {
            ...
            if (delegate == null) {
                final String msg =
                        ...
                        "returned null";
                throw new javax.management.JMRuntimeException(msg);
            }
    ...
    public synchronized static
            ArrayList<javax.management.MBeanServer> findMBeanServer(String agentId) {

        checkPermission("findMBeanServer");

        if (agentId == null)
            return new ArrayList<javax.management.MBeanServer>(mBeanServerList);

        ArrayList<javax.management.MBeanServer> result = new ArrayList<javax.management.MBeanServer>();
        for (javax.management.MBeanServer mbs : mBeanServerList) {
     ...
     * javax.management.MBeanServer#getClassLoaderRepository() server.getClassLoaderRepository()}.
    ...
    private static String mBeanServerId(javax.management.MBeanServer mbs) {
        try {
    ...
    private static final ArrayList<javax.management.MBeanServer> mBeanServerList =
     ...
     **/
    private static javax.management.MBeanServerBuilder newBuilder(Class<?> builderClass) {
        try {
            final Object abuilder = builderClass.newInstance();
            return (javax.management.MBeanServerBuilder)abuilder;
            ...
            final String msg =
                    ...
                    builderClass + ": " + x;
            throw new javax.management.JMRuntimeException(msg, x);
        }
    }
        ...
        try {
            ...
            try {
                ...
                if (builderClassName == null || builderClassName.length() == 0)
                    newBuilderClass = javax.management.MBeanServerBuilder.class;
                else
