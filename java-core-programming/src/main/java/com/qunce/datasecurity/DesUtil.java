package com.qunce.datasecurity;
...
public class DesUtil {
	...
	public static String unPadding80(String data) {
		...
		if (padlen != 0) {
			...
			String tempdatahead = data.substring(0, data.length() - 16);// 除去后8字节的数据
			...
			for (int i = 0; i < 8; i++) {
				...
				if ("80".equals(temp)) {
					if (i == 7)// 最后一个是80
					{
						tempdataend = tempStr.substring(0, tempStr.length() - 2);
					} else {
						int start2 = 2 * (i + 1);
						String temp2 = tempStr.substring(start2, start2 + 2);
						if ("00".equals(temp2)) // 80的后面是00
						{
							tempdataend = tempStr.substring(0, start);
						}
					}
				}
			}
			if (tempdataend.equals("")) {
				if (tempStr.contains("8000")) {
					tempStr = tempStr.substring(0, tempStr.indexOf("8000"));
				}
				tempdataend = tempStr;
			}
			data = tempdatahead + tempdataend;
			return data;
		}
	}
	/*public static String unPadding80(String data) {
...
//		System.out.println(data);
		String data="88573145FD4EBB941AE6414E2B41F46E1C3A821300FBD2814237FAAD2F2A16C3CC89B1FD89B2ADA41A17A192B5A6684E0F27FE7FE076767B6510DF1168F23E32";
		System.out.println(decrypt("181D244714EE2A1A4E1E6C344359D89D181D244714EE2A1A",data));
	}
}
